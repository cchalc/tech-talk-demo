...
```
dbx execute --cluster-name=<name of interactive cluster> --job=tech-talk-demo-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx launch --job=tech-talk-demo-sample-integration-test
```
...
```
dbx launch --job=tech-talk-demo-sample
```
